...
## 4.1 Analysis of English Translations

This sub-section looks at the dataset of 29 translated books.

### 4.1.1 Readability Indices
I used the Flesch-Kincaid formula to test text complexity.


```{=html}
<iframe src="plots/readability_unified.html" width="100%" height="500px" style="border:none;"></iframe>
```
*Figure 4.1.1: Flesch-Kincaid Readability Indices per Novel grouped by Literary School.*
...
### 4.1.2 Stability of Readability Indices over Sliding Windows
...
```
*Figure 4.1.2: Sliding Window Flesch-Kincaid Readability with a window size of 2000 tokens.*
...
```
*Figure 4.1.3: Zipf Distribution per Document plotted on a log-log scale.*
...
```
*Figure 4.1.4: Dialogue vs. Narration.*
...
## 4.2 Analysis of French Originals
This sub-section looks at the control dataset of 6 French books. We used language-specific rules for this pipeline.

### 4.2.1 Readability Indices

We used the Kandel-Moles formula to test French text complexity.

```{=html}
<iframe src="plots/kandel_moles_fr.html" width="100%" height="500px" style="border:none;"></iframe>
```
*Figure 4.2.1: Kandel-Moles Readability (French Originals).*

Figure 4.2.1 shows scores tightly clustered between 84 and 89. Both literary schools have very similar baseline scores.

### 4.2.2 Sliding Window Stability

We applied the same 2000-token window.

```{=html}
<iframe src="plots/sliding_window_fr.html" width="100%" height="600px" style="border:none;"></iframe>```
```
*Figure 4.2.2: Sliding Window Readability(French Originals).*

Figure 4.2.2 shows the same high volatility seen in the English texts. The score for Notre-Dame again drops significantly around the 50k and 100k token marks.

### 4.2.3 Zipf's Law Distribution

We plotted word frequency for the French words.

```{=html}
<iframe src="plots/zipf_fr.html" width="100%" height="500px" style="border:none;"></iframe>
```
*Figure 4.2.3: Zipf Distribution(French Originals).*

Figure 4.2.3 shows all 6 curves perfectly follow the slope of -1.


### 4.2.4 Dialogue vs. Narration

We used a precise Regular Expression rule to extract French dialogue. We targeted French guillemets and em dashes.

```{=html}
<iframe src="plots/dialogue_fr.html" width="100%" height="500px" style="border:none;"></iframe>
```
*Figure 4.2.4: Sentence Length (French Originals).*

Figure 4.2.4 shows the corrected data. Narration is now longer than dialogue. Realist dialogue is significantly shorter than Romantic dialogue.

### 4.2.5 Pure Dialogue Readability

We applied the Kandel-Moles formula only to the extracted French dialogue.

```{=html}
<iframe src="plots/dialogue_readability_fr.html" width="100%" height="500px" style="border:none;"></iframe>
```
*Figure 4.2.5: Pure Dialogue Readability (French Originals).*

Figure 4.2.5 shows the Realist dialogue scores above 100. The Romantic dialogue scores around 98. This data means Realist speech patterns use shorter words and simpler structures.

## 4.3 Cross-Lingual Synthesis

By comparing the English data (Section 4.1) and the French data (Section 4.2), we found three data-driven conclusions.

First, the translation process changes syntactic metrics. In Section 4.1, the English Realist texts showed high variance in readability. However, in Section 4.2, the French originals showed a very tight cluster. This data indicates that the variance in the English texts comes from the translators, not the original authors.

Second, basic structural patterns transcend language. Both datasets strictly follow Zipf's Law with a slope of -1. Both datasets also show identical readability drops in specific books like Notre-Dame.

Third, simple NLP rules are insufficient for complex texts. The basic string-matching rule in the English pipeline created a false anomaly where dialogue appeared longer than narration. Using precise, language-specific extraction rules in the French pipeline fixed this error and revealed the true data structure.
...
highlight_books = {
    "miserables": "blue",    # Romantic
    "notre-dame": "cyan",    # Romantic
    "germinal": "red",       # Realist 
    "l_assommoir": "orange"  # Realist 
}
