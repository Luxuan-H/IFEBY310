...
parallelize → mapPartitions → map → mapPartitions → WholeStageCodegen → WriteFiles. T
here are no shuffle boundaries in this stage. This means Spark completed the entire write operation within a single stage without moving data between partitions.

![](plots/spark_stage_details.png)
