...
subtitle: "A Case Study of Victor Hugo and Émile Zola using Spark NLP"
...
This setting was chosen because the workflow runs in local mode on a single machine. With only one partition, Spark does not need to manage multiple empty partition files. However, no timing comparison between 200 partitions and 1 partition was recorded during this project. The impact of this setting cannot be quantified from the available data.
...
This reduces the number of records entering the shuffle stage. Punctuation marks and single characters are excluded from the word count, which decreases both the shuffle size and memory pressure during aggregation.
