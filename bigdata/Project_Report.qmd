---
title: "Stylometric Analysis of 19th Century French Literature"
subtitle: "Looking at Victor Hugo and Émile Zola using Spark NLP"
...
My project analyzes a total of 35 French novels from 19th century. I divided these books into two main datasets:
...
Both Realist and Romantic novels appear throughout the full range of scores. There is no clear separation between the two literary schools. Some Realist novels score very low, such as l_argent at 64.63, while others score high, such as la_faute at 79.28. The same pattern appears for Romantic novels: bug_jargal scores 72.39 while le_dernier_jour scores 81.91. More than 80 means it is readable by 11-12 years old students, while 64 is for 13-15 years old (higher means easier to read).
...
Four analyses were performed on this corpus. The results are summarized below.

The readability analysis showed that FK scores for the English translations range widely from 64.63 to 81.91, while KM scores for the French originals cluster tightly between 84.46 and 89.45. This difference suggests that translation introduces variance that is not present in the original texts.

The sliding window analysis showed that readability indices are not stable within a single novel. A window of 500 words is too small and produces unreliable scores. A window of 2000 words is more stable but still shows large local drops. 

The Zipf analysis showed that all novels in both languages follow the same power-law distribution regardless of author or literary movement. 

The dialogue analysis showed that the quality of the segmentation method directly affects the results. The French detection rules produced more precise results than the English rules. In the French data, Realist dialogue averages 21 characters per sentence compared to 65 characters for Romantic dialogue.

# 5.Experience with Spark and Big Data File Formats

## 5.1 How Spark Processes the Jobs

The Spark UI was used to observe how Spark executes the jobs in this workflow.

Two types of jobs were examined: the annotation pipeline and the stylometric analysis.

![](plots/spark_dag.png){width="429"}

*Figure 5.1.1: DAG visualization for Stage 33 — Parquet write operation.*

Figure 5.1.1 shows the DAG for Stage 33, which corresponds to the Parquet write operation during annotation. The DAG shows a linear sequence of six steps:

parallelize → mapPartitions → map → mapPartitions → WholeStageCodegen → WriteFiles.

There are no shuffle boundaries in this stage. This means Spark completed the entire write operation within a single stage without moving data between partitions.

![](plots/spark_stage_details.png) *Figure 5.1.2: Stage 38 details — NLP annotation task.*

Figure 5.1.2 shows the details for Stage 38, the NLP annotation task. The stage ran 4 tasks. Each task took between 39 and 41 seconds. The garbage collection time was 4 seconds per task, which is about 10% of the total task duration. This shows that the JVM garbage collector was active during processing, which matches the 1 GB heap limit set for this machine.

## 5.2 Critical Shuffle Identification

A shuffle occurs when Spark needs to redistribute data across partitions. This happens during operations such as `groupBy` and `join`. Two shuffle operations were observed and compared in this workflow.
...
## 5.3 Shuffle Mitigation Strategies

Two strategies were applied to reduce shuffle overhead in this workflow.

Strategy 1: Reducing shuffle partitions By default, Spark uses 200 partitions for shuffle operations. On a single machine with limited memory, this creates 200 tasks that each consume JVM overhead even when most partitions are empty. The following configuration was applied at the start of every Spark session:

``` python
spark.conf.set("spark.sql.shuffle.partitions", "1")
```

A timing experiment was run on the Germinal dataset. Reducing partitions from 200 to 1 reduced execution time from 5.80 seconds to 1.48 seconds, a speedup of 3.9x. On a single machine in local mode, most of the 200 partitions are empty. Managing these empty partitions creates unnecessary overhead. Setting partitions to 1 removes this overhead.

Strategy 2: Filtering before shuffling

In the Zipf pipeline, short tokens were removed before the `groupBy` operation:

``` python
words_df.filter(F.length("word") \> 1).groupBy("book", "word").count()
```

A timing experiment was run on the same dataset. The results are shown in the figure below.

![](plots/5_2.png) *Figure 5.3.1: Timing results for both mitigation experiments on the Germinal dataset.*

The result was unexpected. Filtering before groupBy (1.19s) was slightly slower than without filtering (0.93s). The overhead of the filter operation itself outweighed the benefit of reducing the number of records entering the shuffle. This strategy did not improve performance on this dataset.

## 5.4 Impact of Caching

A cache experiment was performed on the annotated \<<Germinal>\> dataset. The same two aggregations (word frequency count and sentence count) were run twice: once without caching and once after calling `.cache()`.
...
Without caching, Spark reads the Parquet file from disk and recomputes all transformations for every action. With caching, the DataFrame is stored in memory after the first computation. Subsequent actions reuse the in-memory data without any disk I/O. This produced a speedup of 4.05x on a single novel.

This experiment was performed on one novel only due to memory constraints. The machine has 3.7 GB of total RAM and a 1 GB JVM heap. Caching the full 29-novel corpus would likely cause memory pressure and trigger GC pauses. For the same reason, caching was not applied to the full corpus in this workflow. The `.persist()` and `.checkpoint()` methods were not tested due to the same memory constraints.

## 5.5 File Format Comparison

The annotated DataFrame for Germinal was written and read back in three formats.

CSV was not tested because it does not support nested data types such as the `ARRAY\<STRUCT\<...\>\>` columns produced by Spark NLP. Attempting to write the annotated DataFrame to CSV produced the following error:

"The CSV datasource doesn't support the column document of the type ARRAY\<STRUCT\<...\>\>"

It is a limitation of the CSV format, which cannot represent hierarchical data structures.

Thus the results for the other three supported formats are shown as below.

![](plots/parquet.png)

*Figure 5.5.1: Terminal output for parquet write experiment.*

![](plots/json_orc.png)

*Figure 5.5.2: Terminal output for orc and json write experiments.*

Parquet has the best balance between write time and read time. It writes in 17.78 seconds and reads in 1.22 seconds.

ORC produces the smallest file at 6.343 MB, which is 31% smaller than Parquet. However, ORC takes 30.53 seconds to write, which is 71% slower than Parquet.

JSON produces the largest file at 127.338 MB, which is 13.8 times larger than Parquet. Its read time of 7.25 seconds is also 5.9 times slower than Parquet.

These results reflect the differences between the formats. JSON stores data as human-readable text with no compression, which explains its large file size and slow read time. ORC uses aggressive column-level compression, which explains its small file size but slower write time. Parquet uses columnar storage with moderate compression, which gives it fast read times because Spark can skip columns that are not needed for a query. For these reasons, Parquet was chosen as the primary storage format in this project. It supports nested data types, offers fast read times for repeated analysis, and is natively supported by Spark NLP.
...
# Third-party data and plotting packages
import requests
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go

# Spark and Spark NLP packages
import sparknlp
...
for import_name, pip_name in required_packages.items():
    try:
        ...
        print(f"Package '{import_name}' not found. Auto-installing '{pip_name}'...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", pip_name])
        print(f"Successfully installed '{pip_name}'!")

# =====================================================================
# 1、Book definitions: URL and start/end tokens for body extraction
# =====================================================================
...
# =====================================================================
# 2、Download and cleaning pipeline
# ====================================================================
...
# =====================================================================
# 3、Spark NLP Annotation Pipeline
# =====================================================================
...
# ============================================================
# 4 Stylometric Analysis (Unified English Corpus Optimized)
# ============================================================
...
# ============================================================
# 4.1 Readability Indices (Flesch-Kincaid Only)
# ============================================================
...
# ============================================================
# 4.2 Sliding Window Readability 
# ============================================================
...
# ============================================================
# 4.3 Zipf Distribution per Document 
# ============================================================
...
# ============================================================
# 4.4 Dialogue vs. Narration Segmentation
# ============================================================
